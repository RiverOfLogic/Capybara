...
class Config(BaseModel):
    ...
    # 单任务内上下文压缩配置（ReAct 循环内 assistant+tool 消息，见 P1-4）
    run_compaction_token_threshold: Optional[int] = None  # 触发压缩的绝对 token 阈值；None 时取 context_window * compression_threshold
    run_compaction_keep_recent_groups: int = 6  # 压缩时保留最近多少个 assistant+tool 分组不折叠

    # 智能摘要配置
    summary_llm_provider: str = "deepseek"  # 摘要专用 LLM 提供商
    summary_llm_model: str = "deepseek-v4-flash"  # 摘要专用 LLM 模型
    summary_max_tokens: int = 800  # 摘要最大 Token 数
    summary_temperature: float = 0.3  # 摘要生成温度（更确定性）

    # 验证—修复闭环配置
    max_fix_iters: int = 3  # verify_command 失败时自动回灌修复的最大轮数

    # 代码智能子系统（见 docs/code-intelligence.md）
    ctags_path: str = ""            # ctags 可执行文件路径（单一配置来源）；留空则查系统 PATH 上的 "ctags"
    code_index_dir: str = ".agent"  # 索引产物目录（tags / index.db），相对工作区根

    # MCP：作为客户端连接的外部 server 列表（统一在 .capybara.toml 的 [[mcp_servers]] 配置）。
    # 每项字段：name / command / args / env / cwd / timeout / prefix，对齐 CodingAgent._connect_mcp_server。
    ...
    def from_file(cls, path: str) -> "Config":
        """从项目级 TOML 配置文件加载（如 .capybara.toml），字段名对应 Config 字段名。"""
        import tomllib
