...
class SessionStore:
    ...
    def delete(self, session_name: str) -> bool:
        ...
        入参容错：可传裸名（``foo``）、带后缀名（``foo.json``），或
        ``list_sessions()`` 返回的完整 ``filename`` / ``filepath``——
        统一取 basename、去掉 ``.json`` 后缀后再定位文件。

        Args:
            session_name: 会话名称 / 文件名 / 完整路径

        Returns:
            是否删除成功
        """
        base = os.path.basename(str(session_name))
        if base.endswith(".json"):
            base = base[: -len(".json")]
        filepath = self.session_dir / f"{base}.json"
        if filepath.exists():
            os.remove(filepath)
            return True
        return False
